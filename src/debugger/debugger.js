...
import {
  ...
  binToHex,
  binToHexTransformations,
  hexToBinTransformations,
...
function getCreateOptions() {
  ...
  if (cForm.extensions.checkbox.checked) {
    ...
    if (cForm.extensions.credProps.checkbox.checked) {
      extensions.credProps = cForm.extensions.credProps.checkbox.checked;
    }
    ...
    if (cForm.extensions.minPinLength.checkbox.checked) {
      extensions.minPinLength = cForm.extensions.minPinLength.checkbox.checked;
    }

    if (cForm.extensions.uvm.checkbox.checked) {
      extensions.uvm = cForm.extensions.uvm.checkbox.checked;
    }
...
function handleRegistrationCredentials(credentials) {
  ...
  if (
    lastCredentialsParsed.response.attestationObject &&
...
function getGetOptions() {
  ...
  if (gForm.extensions.checkbox.checked) {
    ...
    if (gForm.extensions.uvm.checkbox.checked) {
      extensions.uvm = gForm.extensions.uvm.checkbox.checked;
    }
...
function downloadRaw() {
  ...
  const transformations = deepClone(binToHexTransformations);

  transform(creds, transformations);

  const stringify = prettyStringify(creds);

  //log.debug(encoded);
  saveAs(new Blob([stringify]), "raw-output.json");

  // const obj = JSON.parse(stringify);

  // const decodeTransformations = deepClone(hexToBinTransformations);

  // transform(obj, decodeTransformations);
...
function setupEvents() {
  ...
  dom.output.downloadRaw.forEach((button) =>
    button.addEventListener("click", downloadRaw)
  );
